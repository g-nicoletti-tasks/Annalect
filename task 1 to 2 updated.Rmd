...
Answer: R2 (and Adjusted R2) are often used for explanatory purposes. The advantage of having training and testing datasets is that we can easily understand how well the model predicts new observations not included in the training dataset.
Because the task asks for a measure of the relationship between the display_adstocked and the number of orders, then the R2 would be my choice, BUT I need to be careful because R2 in training and testing are incredibly different! So maybe better to double-check this before taking the final decision!
...
Answer: I would probably consider building a function that can read and combine many variables and transformations simultaneously (maybe using purrr::map() function) - warning: this function may be long to build and potentially very slow to run.
